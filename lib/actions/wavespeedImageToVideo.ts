...
async function requireSignedIn(token?: string | null): Promise<{ error?: string }> {
  const user = await authenticateSlutbotToken(token);
...
export async function uploadImageToVideoSource(
  ...
  const auth = await requireSignedIn(token);
...
export async function deleteAiToolGeneration(
  id: string,
  clientId: string,
  token?: string | null,
  ...
  if (!trimmedId || !trimmedClientId) return { ok: false, error: 'Missing id.' };

  try {
    await connectDB();
    const result = await AiToolGeneration.deleteOne({ _id: trimmedId, clientId: trimmedClientId });
...
export async function listAiToolGenerations(
  ...
  try {
    await connectDB();
    const docs = await AiToolGeneration.find({ clientId: trimmedClientId })
      .sort({ createdAt: -1 })
      .limit(100)
      .lean();
